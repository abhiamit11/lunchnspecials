...
import { AboutSchema, AboutType } from "./schema";
...
function AboutUs() {
  ...
  const { data, isSuccess, refetch } = useQuery({
    queryKey: ["about-us-content"], //Array according to Documentation
  ...
  const onTextChange = () => {
    if (quillRef.current && quillRef.current.root) {
      form.setValue('aboutHtml', quillRef.current.root.innerHTML)
    }
  }
  const [showPreview, setShowPreview] = useState(true)


  const onSubmit = (data: AboutType) => {
    mutate(data)
  }
  ...
  const value = form.watch("aboutHtml") || data?.content || '';
    ...
    <>
      <div className="flex justify-between items-center py-4 rounded-lg">
        <div className="flex justify-start items-center gap-2">
          ...
          <h1 className="text-2xl font-medium">About Us Page</h1>
        </div>
